...
def scanCAS(lines):
    ...
    while casStream != "":
        ...
        if not proc:
            raise Exception([{"name": "scanCAS", "msg": "no value to keyword " + kw}])
        val = []
        while proc:
            if proc.group("val") == '"':
                val.append("")
...
def readCAS(cas, dico, frgb):
    ...
    for key, value in zip(*keywords):
            ...
            if kw not in frgb["GB"]:
                raise Exception(
                    [
                        {
                            "name": "readCAS",
                            "msg": "... could not find the following keyword in the selected dictionary file: "
                            + key
        ...
        if dico[kw]["TYPE"][0] == "LOGIQUE":
            vals = []
            for val in value:
                if val.upper() in ["YES", "Y", "TRUE", "OUI", "O", "VRAI"]:
                    vals.append("TRUE")
                elif val.upper() in ["NO", "N", "FALSE", "NON", "N", "FAUX"]:
                    vals.append("FALSE")
                else:
                    raise Exception(
                        [
                            {
                                "name": "readCAS",
                                "msg": "... I am looking for a LOGICAL but found an inapropriate value set for keyword: "
                                + key,
                            }
                        ]
                    )
                ...
                if re.match(vint, val):
                    ...
                    raise Exception(
                        [
                            {
                                "name": "readCAS",
                                "msg": "... I am looking for an INTEGER but found an inapropriate value set for keyword: "
                                + key,
                            }
                        ]
                    )
                ...
                if re.match(vflt, val.lower().replace("d", "e")):
                    ...
                    raise Exception(
                        [
                            {
                                "name": "readCAS",
                                "msg": "... I am looking for an FLOAT but found an inapropriate value set for keyword: "
                                + key,
                            }
                        ]
                    )
            ...
            vals = []
            for val in value:
                vals.append(repr(val.replace('"', "''")).replace('"', "'"))
            vallist[keylist.index(key)] = vals
...
def rewriteCAS(cas):
    ...
    for key, val in zip(*cas[1]):
        if val == []:
            raise Exception([{"name": "rewriteCAS", "msg": "... inapropriate value set for keyword: " + key}])
        ...
        for v in val[1:]:
            if len(lcur + ";" + str(v)) < 72:
                ...
                if len(lcur) < 72:
                    ...
                    print "... warning: CAS file cannot read this value: ", lcur
...
def getKeyWord(key, cas, dico, frgb):
    ...
    if key in frgb["GB"]:
        ...
        if key in kl:
            value = vl[kl.index(key)]
        elif frgb["GB"][key] in kl:
            value = vl[kl.index(frgb["GB"][key])]
    if key in frgb["FR"]:
        defaut = dico[key]["DEFAUT"]
        if key in kl:
            value = vl[kl.index(key)]
        elif frgb["FR"][key] in kl:
            value = vl[kl.index(frgb["FR"][key])]
...
def getCASLang(cas, frgb):
    ...
    i = 0
    while kl[i][0] == "&" or (kl[i] in frgb["FR"] and kl[i] in frgb["GB"]):
        i += 1
    if kl[i] not in frgb["FR"]:
        lang = 2
...
def setKeyValue(key, cas, frgb, value):
    ...
    if key in frgb["GB"] and lang == 1:
        key = frgb["GB"][key]
    if key in frgb["FR"] and lang == 2:
        key = frgb["FR"][key]
    kl, vl = keys
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-k", "--rank", type="string", dest="rank", default="all", help="the suite of validation ranks (all by defult)"
    )
    ...
    for cfgname in cfgs:
        ...
        if options.rank != "":
            cfgs[cfgname]["val_rank"] = options.rank
