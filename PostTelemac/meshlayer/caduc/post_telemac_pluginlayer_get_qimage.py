...
class Selafin2QImage:
        ...
        try:
            if self.selafinlayer != None and self.selafinlayer.hydrauparser != None:
                ...
                self.triangulation = matplotlib.tri.Triangulation(
                    self.meshxreprojected, self.meshyreprojected, np.array(ikle)
                )
        except Exception, e:
    ...
    def getTransformedCoords(self, xcoords, ycoords, direction=True):
        ...
        if direction:
            ...
            transformedCoordinatesAsPoints = [
                self.selafinlayer.xform.transform(point, qgis.core.QgsCoordinateTransform.ReverseTransform)
    ...
    def getimage(self, selafinlayer, rendererContext):
        ...
        if ct:
            ...
            rect2 = [
                float(recttemp2.xMinimum()),
                float(recttemp2.xMaximum()),
                float(recttemp2.yMinimum()),
                float(recttemp2.yMaximum()),
            ]
        ...
        if DEBUG and False:  # debug thing
            txt = (
                ...
                + "\n"
                + " px "
                + str(selafinlayer.renderersizepx)
                + "/"
                + str(sizepx)
                + "\n"
                + " temps "
                + str(selafinlayer.temps_memoire)
                + "/"
                + str(selafinlayer.time_displayed)
                + "\n"
                + "param "
                + str(selafinlayer.param_memoire)
                + " "
                + str(selafinlayer.param_displayed)
            )
        ...
        try:
            ...
            if (
                ...
                and self.previousdrawtime == selafinlayer.time_displayed
                and self.previousdrawparam == selafinlayer.param_displayed
                and selafinlayer.lvl_contour == self.previousdrawlvl
                and selafinlayer.alpha_displayed == self.previousdrawalpha
                and self.previousdrawcmcontour == selafinlayer.cmap_mpl_contour_raw
                and selafinlayer.forcerefresh == False
            ):
            ...
            # Case 2 : figure changed (time,param) with the same mapcanvas dimension
                ...
                if not self.tritemp:  # create temp triangulation
                    ...
                    if DEBUG:
                        time1.append("tritemp : " + str(round(time.clock() - timestart, 3)))
