    ...
    def makeFilm(self):
        try:
            ...
            for composeurview in qgis.utils.iface.activeComposers():
                if (
                    composeurview.composerWindow().windowTitle()
                    == self.pluginlayer.propertiesdialog.comboBox_compositions.currentText()
                ):
                    ...
                    if self.fig != None:
                        try:
                            if False:
                                ...
                                self.addtimelineonax(ax, self.pluginlayer.hydrauparser.getTimes()[i])
                                # saving the figure
                                matplotlibimagepath = os.path.join(
                                    self.tempdir, "test" + "%04d" % compt + "." + mplformat
                                )
                                # print str(matplotlibimagepath)
