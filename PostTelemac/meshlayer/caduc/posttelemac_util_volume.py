...
class computeVolume(QtCore.QObject):
    def __init__(self, selafin, line):
        ...
        for i, region in enumerate(regions):
            if -1 not in region and len(region) > 0:
                ...
                if qgspolygonvoronoi.within(self.qgspolygone):
                    ...
                    self.status.emit(
                        "Region : " + str(region) + " - Point lie : " + str(linkedpoint) + " - surface : " + str(area)
                    )
    ...
    def getPointsOutsidePolygon(self, polygon, indexpoints, points):
        ...
        for goodnum in goodnums:
            if QgsGeometry.fromPolygon([[QgsPoint(xMesh[i], yMesh[i]) for i in mesh[goodnum]]]).intersects(
                qgspolygontoline
            ):
                goodnums2.append(goodnum)
    ...
    def getTrianglesWithinPolygon(self, polygon):
        ...
        goodnums2 = []
        for goodnum in goodnums:
            if QgsGeometry.fromPolygon([[QgsPoint(xMesh[i], yMesh[i]) for i in mesh[goodnum]]]).within(
                self.qgspolygone
            ):
                goodnums2.append(goodnum)
