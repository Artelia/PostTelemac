...
class GLViewWidget(QtOpenGL.QGLWidget):
    ...
    def pan(self, dx, dy, dz, relative=False):
        ...
        if not relative:
            ...
            xDist = (
                dist * 2.0 * np.tan(0.5 * self.opts["fov"] * np.pi / 180.0)
    ...
    def renderToArray(self, size, format=GL_BGRA, type=GL_UNSIGNED_BYTE, textureSize=1024, padding=256):
        ...
        try:
            ...
            ## create teture
            glTexImage2D(
                GL_TEXTURE_2D, 0, GL_RGBA, texwidth, texwidth, 0, GL_RGBA, GL_UNSIGNED_BYTE, data.transpose((1, 0, 2))
            )
            ...
            for x in range(-padding, w - padding, texwidth - p2):
                for y in range(-padding, h - padding, texwidth - p2):
                    ...
                    glfbo.glFramebufferTexture2D(
                        glfbo.GL_FRAMEBUFFER, glfbo.GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, tex, 0
                    )
                    ...
                    data = (
                        np.fromstring(data, dtype=np.ubyte).reshape(texwidth, texwidth, 4).transpose(1, 0, 2)[:, ::-1]
                    )
