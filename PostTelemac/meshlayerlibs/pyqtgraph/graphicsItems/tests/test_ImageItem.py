...
def test_ImageItem(transpose=False):
    ...
    assertImageApproved(
        w, "imageitem/gradient_rgba_byte_levels", "RGBA byte gradient. Levels set to show x=128 and y>128."
    )
    ...
    assertImageApproved(
        w, "imageitem/gradient_rgba_float_additive", "RGBA float gradient with alpha and additive composition mode."
    )
    ...
    assertImageApproved(
        w, "imageitem/resolution_with_downsampling_x", "Resolution test with downsampling axross x axis."
    )
    assert img._lastDownsample == (4, 1)

    img.setImage(data.T, levels=[-1, 1])
    assertImageApproved(
        w, "imageitem/resolution_with_downsampling_y", "Resolution test with downsampling across y axis."
    )
...
def test_dividebyzero():
    ...
    im.view.setRange(xRange=[-5 + 25, 5e25], yRange=[-5e25, 5e25])
    app.processEvents()
