...
class ROI(GraphicsObject):
    ...
    **Signals**
    ...
    def __init__(
        ...
        self.freeHandleMoved = (
            False  ## keep track of whether free handles have moved since last change signal was emitted.
        )
    ...
    def movePoint(self, handle, pos, modifiers=QtCore.Qt.KeyboardModifier(), finish=True, coords="parent"):
        ...
        if h["type"] == "t":
            ...
            if self.rotateSnap or (modifiers & QtCore.Qt.ControlModifier):
                ang = round(ang / 15.0) * 15.0  ## 180/12 = 15
    ...
    **Arguments**
    ...
    def getArrayRegion(self, arr, img=None, axes=(0, 1), **kwds):
        ...
        mask = np.fromfunction(
            lambda x, y: (((x + 0.5) / (w / 2.0) - 1) ** 2 + ((y + 0.5) / (h / 2.0) - 1) ** 2) ** 0.5 < 1, (w, h)
        )
...
class PolygonROI(ROI):
    ...
    def boundingRect(self):
        ...
        for h in self.handles:
            r |= self.mapFromItem(
                h["item"], h["item"].boundingRect()
            ).boundingRect()  ## |= gives the union of the two QRectFs
        return r
    ...
    def addSegment(self, h1, h2, index=None):
        ...
        for h in seg.handles:
            ...
            h["item"].setAcceptedMouseButtons(
                h["item"].acceptedMouseButtons() | QtCore.Qt.LeftButton
    ...
    **Arguments**
    ...
    def getArrayRegion(self, data, img, axes=(0, 1), order=1, **kwds):
        ...
        for i in range(len(imgPts) - 1):
            ...
            o = Point(imgPts[i])
            r = fn.affineSlice(
                data, shape=(int(d.length()),), vectors=[Point(d.norm())], origin=o, axes=axes, order=order, **kwds
            )
            rgns.append(r)
