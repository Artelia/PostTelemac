...
class RangeFilterItem(ptree.types.SimpleParameter):
    ...
    def generateMask(self, data, mask):
        ...
        mask[mask] = (vals >= self["Min"]) & (
            vals < self["Max"]
        )  ## Use inclusive minimum and non-inclusive maximum. This makes it easier to create non-overlapping selections
        return mask

    def describe(self):
        return "%s < %s < %s" % (
            fn.siFormat(self["Min"], suffix=self.units),
            self.fieldName,
