    ...
    def remoteSceneChanged(self, data):
        ...
        if self.shm is None or self.shm.size != size:
            ...
            if sys.platform.startswith("win"):
                ...
                self.shm = mmap.mmap(
                    -1, size, self.shmtag
                )  ## can't use tmpfile on windows because the file can only be opened once.
            ...
            if USE_PYSIDE:
                ...
                try:
                    self.img = QtGui.QImage(
                        sip.voidptr(address), self.width(), self.height(), QtGui.QImage.Format_ARGB32
                    )
                except TypeError:
                    try:
                        self.img = QtGui.QImage(
                            memoryview(buffer(self.shm)), self.width(), self.height(), QtGui.QImage.Format_ARGB32
                        )
                    except TypeError:
                        # Works on PyQt 4.9.6
