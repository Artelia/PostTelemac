...
def reloadAll(prefix=None, debug=False):
    ...
    for modName, mod in list(sys.modules.items()):  ## don't use iteritems; size may change during reload
        ...
        if (
            py not in changed
            and os.path.isfile(pyc)
            and os.path.isfile(py)
            and os.stat(pyc).st_mtime >= os.stat(py).st_mtime
        ):
            # if debug:
        ...
        changed.append(
            py
        )  ## keep track of which modules have changed to insure that duplicate-import modules get reloaded.
...
"""
    ...
    b1.fn()
    print(
        "function IDs  a1 bound method: %d a1 func: %d  a1 class: %d  b1 func: %d  b1 class: %d"
        % (
            id(a1.fn),
            id(a1.fn.__func__),
            id(a1.fn.__self__.__class__),
            id(b1.fn.__func__),
            id(b1.fn.__self__.__class__),
        )
    )
