...
class FlowTool(AbstractMeshLayerTool, FORM_CLASS):
    ...
    def createRubberband(self):
        self.rubberband = qgis.gui.QgsRubberBand(self.meshlayer.canvas, qgis.core.QgsWkbTypes.LineGeometry)
            ...
            if len(x) > 1:
                ...
                if sys.version_info.major == 2:
                    self.meshlayer.rubberband.rubberbandface.addGeometry(
                        qgis.core.QgsGeometry.fromPolygon([points]), None
                    )
                elif sys.version_info.major == 3:
                    self.meshlayer.rubberband.rubberbandface.addGeometry(
                        ...
                        if False:
                            results = np.array(
                                self.selafinlayer.hydrauparser.getTimeSerie(
                                    (np.array(shortest) + 1).tolist(),
                                    [parameterh, parameteruv, parametervv],
                                ...
                                if True:
                                    ...
                                    try:
                                        if i == 0:  # init
                                            ...
                                            # xy2 = list( self.selafinlayer.hydrauparser.getXYFromNumPoint([elem])[0] )
                                            xy2 = list(
                                                self.selafinlayer.hydrauparser.getFaceNodeXYFromNumPoint([elem])[0]
                                            )
                                        else:
                                            h1 = h2
                                            v1vect = v2vect
        ...
        for collection in triplotcontourf.collections:
            for path in collection.get_paths():
                for polygon in path.to_polygons():
                    if sys.version_info.major == 2:
                        polygons2 = qgis.core.QgsGeometry.fromPolygon(
                            [[qgis.core.QgsPoint(i[0], i[1]) for i in polygon]]
                        )
                    elif sys.version_info.major == 3:
            ...
            if templine.type() == 1:
                ...
                if not templine.isMultipart():
                    ...
                    for line3 in templine.asMultiPolyline():
                        if sys.version_info.major == 2:
                            linefinal2.append(
                                qgis.core.QgsGeometry.fromPolyline([qgis.core.QgsPoint(i[0], i[1]) for i in line3])
                            )
                        elif sys.version_info.major == 3:
                            linefinal2.append(
