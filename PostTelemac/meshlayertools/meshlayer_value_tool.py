    ...
    def valeurs_click(self,qgspointfromcanvas):
        ...
        if self.comboBox_values_method.currentIndex() == 0 :
            if int(qgis.PyQt.QtCore.QT_VERSION_STR[0]) == 4:
                ...
                qgspoint= self.meshlayer.xform.transform(qgis.core.QgsPointXY(qgspointfromcanvas[0],qgspointfromcanvas[1]),qgis.core.QgsCoordinateTransform.ReverseTransform)
            ...
            if True:
                ...
                for i, param in enumerate(self.meshlayer.hydrauparser.parametres):
                    if param[2] == 0 :
                        showelem = True
                        self.tableWidget_values.setItem(i, 1, QTableWidgetItem(str(round(self.meshlayer.values[i][numnearestelem],3))))
                    elif param[2] == 1 :
                        showfacenode = True
                        self.tableWidget_values.setItem(i, 1, QTableWidgetItem(str(round(self.meshlayer.values[i][numnearestfacenode],3))))
                    elif param[2] == 2 :
                        showface = True
                        self.tableWidget_values.setItem(i, 1, QTableWidgetItem(str(round(self.meshlayer.values[i][numnearestface],3))))
