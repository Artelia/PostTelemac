    ...
    def computeVolume(self):
        ...
        if self.selectionmethod in [0]:
            ...
            if not (layer.type() == 0 and layer.geometryType() == 2):
                QMessageBox.warning(
                    qgis.utils.iface.mainWindow(), "PostTelemac", self.tr("Select a polygone vector layer")
                )
            elif self.selectionmethod == 1 and len(layer.selectedFeatures()) == 0:
                QMessageBox.warning(
                    qgis.utils.iface.mainWindow(), "PostTelemac", self.tr("Select a polygon in a polygon vector layer")
                )
            else:
                # self.ax.cla()
                    ...
                    for geoms in geomss:
                        ...
                        for geom in geoms:
                            qgspoint = xformutil.transform(
                                QgsPoint(geom[0], geom[1]), qgis.core.QgsCoordinateTransform.ReverseTransform
                            )
            ...
            if len(self.pointstoDraw) > 0:
                ...
                for i in range(0, len(self.pointstoDraw)):
                    self.meshlayer.rubberband.rubberbandface.addPoint(
                        qgis.core.QgsPoint(self.pointstoDraw[i][0], self.pointstoDraw[i][1])
                    )
        ...
        if self.selectionmethod == 0:
            ...
            for point in self.pointstoDraw:
                qgspoint = xform.transform(
                    qgis.core.QgsPoint(point[0], point[1]), qgis.core.QgsCoordinateTransform.ReverseTransform
                )
...
class computeVolume(QtCore.QObject):
    def __init__(self, selafin, tool, line):
    ...
    def computeVolumeVoronoiQGis(self, METHOD, points, indexpoints):
        ...
        for (site, edges) in list(c.polygons.items()):
            ...
            if not qgsvoropolygone.within(self.qgspolygone):
                intersectedpolyg = qgis.core.QgsGeometry.fromPolygon(
                    qgsvoropolygone.intersection(self.qgspolygone).asPolygon()
                )
            else:
        ...
        for i, result in enumerate(voronoipolyg):
            ...
            if METHOD == 1:
                h = np.array(
                    self.selafinlayer.hydrauparser.getTimeSerie([result[0] + 1], [parambottom, paramfreesurface])
                )
                ...
                h = np.array(
                    self.selafinlayer.hydrauparser.getTimeSerie(
                        [result[0] + 1], [self.selafinlayer.propertiesdialog.comboBox_volumeparam.currentIndex()]
                    )
                )
    ...
    def getPointsinPolygon(self, polygon):
        ...
        for goodnum in goodnums:
            if qgis.core.QgsGeometry.fromPoint(qgis.core.QgsPoint(xMesh[goodnum], yMesh[goodnum])).within(
                self.qgspolygone
            ):
    ...
    def getPointsOutsidePolygon(self, polygon, indexpoints, points):
        ...
        for goodnum in goodnums:
            if qgis.core.QgsGeometry.fromPolygon(
                [[qgis.core.QgsPoint(xMesh[i], yMesh[i]) for i in mesh[goodnum]]]
    ...
    def computeVolumeMesh(self, METHOD, indextriangles):
        ...
        for i, indextriangle in enumerate(indextriangles):
            ...
            if METHOD == 0:
                h = np.array(
                    self.selafinlayer.hydrauparser.getTimeSerie(
                        [mesh[indextriangle, 0] + 1, mesh[indextriangle, 1] + 1, mesh[indextriangle, 2] + 1],
                        [parambottom, paramfreesurface],
                    )
                )
                ...
                h = np.array(
                    self.selafinlayer.hydrauparser.getTimeSerie(
                        [mesh[indextriangle, 0] + 1, mesh[indextriangle, 1] + 1, mesh[indextriangle, 2] + 1],
                        [self.selafinlayer.propertiesdialog.comboBox_volumeparam.currentIndex()],
                    )
                )
