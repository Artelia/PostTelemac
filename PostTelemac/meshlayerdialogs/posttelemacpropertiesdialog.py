...
class PostTelemacPropertiesDialog(QDockWidget, FORM_CLASS):
    ...
    def update(self):
        ...
        if self.meshlayer.hydraufilepath is not None:
            ...
            if self.meshlayer.parametrestoload['renderer'] is not None :
                ...
                #user
                index = self.comboBox_clrramp_preset.findText(self.meshlayer.parametrestoload['renderer'][3], QtCore.Qt.MatchFixedString)
                if index > 0 :
    ...
    def loadSelafin(self):
        ...
        if int(qgis.PyQt.QtCore.QT_VERSION_STR[0]) == 4:
            tempname, extension = self.qfiledlg.getOpenFileNameAndFilter(None, 'Choose the file', self.loaddirectory,
                                                                         str1, str1.split(';;')[0])
        ...
        if tempname:
            ...
            if success:
                ...
                self.normalMessage(self.tr('File ') + str(nom) + self.tr(" loaded in ") + str(round(time.clock() - timestart,1)) + ' s')
        else:
