...
class PyGLWidget(QtOpenGL.QGLWidget):
    ...
    def loadVertexes(self):
        ...
        self.centermesh = np.array([( max(self.meshx) + min(self.meshx) ) / 2 ,( max(self.meshy) + min(self.meshy) ) / 2 ,0.0]) 
    ...
    def changeFreeSurfaceVertex(self,time):
        if False:
            self.vtxtodrawwater = ( np.stack((self.meshx, 
                                    ...
                                    axis=-1)   ) 
            self.iklewater = self.parser.getIkle()
    ...
    def paintGL(self):
        ...
        #glTranslatef(-421431 , -6435862,  0)
        
        if False :   #working
            glBegin(GL_TRIANGLES)
            glColor3f(1,0,0)
            for pairr in self.meshlayer.hydrauparser.getIkle():
                #print 'pari ' + str(pairr)
                #glColor3f(1,0,0)
                glVertex3f(self.vtxtodraw[pairr[0],0],self.vtxtodraw[pairr[0],1], self.vtxtodraw[pairr[0],2] )
                #glColor3f(0,1,0)
                glVertex3f(self.vtxtodraw[pairr[1],0],self.vtxtodraw[pairr[1],1], self.vtxtodraw[pairr[1],2] )
                #glColor3f(0,0,1)
                glVertex3f(self.vtxtodraw[pairr[2],0],self.vtxtodraw[pairr[2],1], self.vtxtodraw[pairr[2],2] )
            glEnd()
        ...
        if False :   #working
            ...
            glDrawElementsui(GL_TRIANGLES, self.meshlayer.hydrauparser.getIkle())
    ...
    def rotate(self, _axis, _angle):
        ...
        if True :
            #center to modelview
            t = np.dot( np.transpose( self.modelview_matrix_ )   ,np.array([ self.center_[0], self.center_[1], self.center_[2],1.0    ]) )
            
            #print 't \n' + str(t)
             
            #print 't1 \n' + str(  np.dot( np.transpose( self.modelview_matrix_ )   ,np.array([ self.center_[0], self.center_[1], self.center_[2],1.0    ])          ) )

        self.makeCurrent()
        if True:
            glLoadIdentity()
            glTranslatef(t[0], t[1], t[2])
            #glTranslatef(self.center_[0] , self.center_[1], self.center_[2])
    ...
    def view_all(self):
        ...
        if False:
            ...
            print 'view 1 ' + str( [ -( self.modelview_matrix_[0][0] * self.center_[0] +
            ...
            #t =  np.dot(numpy.linalg.inv ( np.transpose(  np.array(glGetDoublev(GL_MODELVIEW_MATRIX) )  )  ) , np.array([ -self.center_[0], -self.center_[1], -self.center_[2],1.0    ] ) )
    ...
    def getRay(self,mousex = None, mousey=None):
        ...
        if True :       #get Ray
            if mousex != None and mousey != None:
                ...
                transposedmatrix = np.transpose( np.array(glGetDoublev( GL_PROJECTION_MATRIX) )  )  
    ...
    def getNearestPointFromRay(self,eye, ray_eye):
        ...
        #print 'point \n' + str(self.vtxtodraw[temp1][0])
        #self.center_ = self.vtxtodraw[temp1][0]
...
class Window(QtGui.QWidget):
    ...
    def createButton(self):
        ...
        button.clicked.connect(self.tete)
        return button
        
    def tete(self):
        self.glWidget.view_all()
