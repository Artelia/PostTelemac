...
class PostTelemacHDFParser(PostTelemacAbstractParser):
    ...
    def getElemRawValue(self, time1):
        ...
        if self.bottomvalue == None:
            ...
            if DEBUGTIME : print('file subdata read : ' + str(round(time.clock()-timestart,3)) )
            ...
            self.bottomvalue = np.transpose(np.repeat(np.array(finalbottomvalue), (self.itertimecount +1)).reshape((-1,( self.itertimecount +1) ) ) )
        ...
        if DEBUGTIME : print('fbottom value : ' + str(round(time.clock()-timestart,3)) )
                ...
                for param in self.varnames:
                    if param[1] == 0:
                        ...
                        if param[3] != None:
                            for name in param[3]:
                                if False:
                                    if DEBUGTIME : print('param : ' +str(param[0]) + ' begin ' + str(round(time.clock()-timestart,3))  )
                                    band_ds = gdal.Open(name, gdal.GA_ReadOnly)
                                    array = band_ds.ReadAsArray()
                                    band_ds = None
                                    if DEBUGTIME : print('param : ' +str(param[0]) + ' array read ' + str(round(time.clock()-timestart,3)) )
                                    if np.array(array).shape[0] == 2:
                                        tempval = np.concatenate((tempval,array[0]))
                                    else:
                                        tempval = np.concatenate((tempval,array[time1]))
                                    if DEBUGTIME : print('param : ' +str(param[0]) + ' concatenate ' + str(round(time.clock()-timestart,3)) )
                            ...
                            if DEBUGTIME : print('param : ' +str(param[0]) + ' append raw value ' + str(round(time.clock()-timestart,3))  )
                
                if DEBUGTIME : print('done : '  + str(round(time.clock()-timestart,3)) )
                            ...
                            if True:
                                ...
                                if array.shape[0] == 2:
                                    if tempval == None:
                                        ...
                                        #tempval = array[0]
                                        tempval = np.transpose(np.repeat(np.array(array[0]), (self.itertimecount +1 )  ).reshape((-1,( self.itertimecount  +1) )) )

                                    else:
                                        temp1 = np.transpose(np.repeat(np.array(array[0]), (self.itertimecount + 1  ) ).reshape((-1,( self.itertimecount + 1)  )) )
                                        #tempval = np.concatenate((tempval,array[0]))
